...
- Опция `storage.keep_audio_ttl` существует в конфиге и по умолчанию `0` (выключено).
...
```
                      ┌──────────────────────── HTTP :8080 ────────────────────────┐
                      │  middleware: requestid → recover → auth → limits → cors    │
                      └───────┬───────────────────┬──────────────────┬─────────────┘
                              │                   │                  │
                 ┌────────────▼───────┐ ┌─────────▼────────┐ ┌───────▼────────┐
...
notes: "8 кГц телефония: apply denoiser=false, best with beam=4"
```
...
- **Каталог** — `catalog.yaml`, вшит в бинарь (`embed.FS`), может быть переопределён
  `registry.catalog_url` (своё зеркало / закрытый контур).
...
`decoding_method`, `max_active_paths`, `response_format`, `webhook_url`.
...
| 429 | `queue_full` / `rate_limited` | + `Retry-After` |
...
- Аудио в БД **не хранится** (решение №15). `filename`/`audio_bytes` — только метаданные.
- При старте: незавершённые `running` помечаются `failed` c `error_code=server_restart`
  (задачу нельзя восстановить — аудио уже удалено). Это осознанное следствие решения №15
  и должно быть в документации API.
...
storage:
  ...
  keep_audio_ttl: 0                # 0 = не хранить (решение №15)
...
| SSRF | `webhook_url` — только HTTPS, запрет приватных диапазонов (RFC1918, link-local, loopback) по умолчанию |
| Path traversal | `model_id` валидируется regex `^[a-zA-Z0-9._-]{1,64}$`, склейка путей только через `filepath.Join` + проверка префикса |
| UI | по умолчанию `require_auth: true`; отключается конфигом и полностью вырезается build-тегом `noui` |
...
### 12.1 Ориентиры RTF (проверить на своём железе — см. 19.1)
...
| Примитивы | Base UI (`@base-ui-components/react`) |
...
| `no-restricted-imports` | импорт `@base-ui-components/react/*` вне `src/components/ui/**` |
...
- **Пики** считаются в Web Worker: `decodeAudioData` → RMS по окнам → `Float32Array`
  (~2000 точек на весь файл). Для 10 минут это доли секунды и не блокирует UI-поток.
...
- Экспорт: JSON, SRT, VTT, TXT — генерируется **на клиенте** из уже полученного результата.
...
| `/settings` | Язык, тема, ключ API (для `require_auth`), пороги подсветки |
...
│   ├── httpx/              # middleware: requestid, recover, auth, limits, cors
...
| **M1 — вертикальный срез** | WAV-декодер → VAD → transducer → слова → `/v1/audio/transcriptions` (json/verbose_json), одна модель из локальной папки | 5 |
| **M2 — модельный слой** | манифесты, каталог, скачивание с проверкой sha256, LRU-пул, refcount, hot swap, CPU-губернатор, семейства CTC | 6 |
| **M3 — очередь и API** | SQLite, jobs, SSE, отмена, история, нативный диалект, RFC 9457, ключи и scope, лимиты, `srt`/`vtt` | 6 |
| **M4 — UI** | оболочка, стена ESLint/TS, примитивы, тема, i18n, скелетоны, поверхности, история уведомлений, плеер, менеджер моделей, история | 10 |
...
| 5 | Oversubscription потоков onnxruntime | высокая | среднее | CPU-губернатор (12.2), проверка на старте |
...
| 8 | 30-минутные файлы × параллелизм съедают память | средняя | высокое | 115 МБ/задача учтено в авто-дефолтах, проверка на старте (12.3) |
...
2. **Какая точность word-таймингов считается приемлемой?** ±50 мс, ±100 мс, ±200 мс?
   От этого зависит, нужен ли forced alignment в v1.
...
5. Какая модель — **дефолтная** (`asr.default_model`)?
