...
### 12.2 Ориентиры RTF для других моделей (проверить на своём железе)
...
| 5 | Oversubscription потоков onnxruntime | высокая | среднее | CPU-губернатор (12.3), проверка на старте |
...
| 8 | 30-минутные файлы × параллелизм съедают память | средняя | высокое | 115 МБ/задача учтено в авто-дефолтах, проверка на старте (12.4) |
...
2. **Какая точность word-таймингов считается приемлемой?** ±50 мс, ±100 мс, ±200 мс?
   Частично закрыто на M1: узкополосность добавляет ≤48 мс (см. 12.1). Абсолютная
