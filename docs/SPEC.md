...
Решения 34–38 приняты в M5, когда постобработка и диаризация впервые
столкнулись с тем, что библиотека действительно умеет.

| # | Решение | Последствие |
|---|---|---|
| 34 | **Диаризация неотменяема внутри одного прохода** | В C-API есть `ProcessWithCallback`, Go-обёртка его не экспортирует. Отменённая задача переживает свою отмену на длину прохода (~0.05–0.5× RTF). Прогресса внутри стадии тоже нет |
| 35 | **`hotwords`, `decoding_method`, `max_active_paths` — свойства времени загрузки** | sherpa фиксирует их при конструировании распознавателя, per-stream API в Go-обёртке нет, а `SetConfig` мутирует общий экземпляр. Честно их исполнить можно только вторым резидентным экземпляром: ключ пула несёт вариант, `asr.variants.max` — бюджет, по умолчанию 0 и варнинг вместо тишины |
| 36 | **Русская пунктуация приходит от модели ASR, а не от CT-Transformer** | Русского CT-Transformer не существует. GigaAM v3 punct пишет знаки и регистр сам, и он стал моделью по умолчанию. `postproc.punctuation` остаётся для моделей, которые сами не пунктуируют |
| 37 | **`punctuation_builtin` измеряется по словарю модели** | Из семейства это не выводится: v2 и v3 — одно семейство и различаются ровно этим. Манифест возможностей не заявляет (§7.1), а `tokens.txt` уже открыт при загрузке и является собственным свидетельством модели |
| 38 | **Сегменты режутся по границам тёрнов при `diarize=true`** | §5.7 требует пословной привязки, §6 держит спикера на сегменте. Резка — единственный способ сделать оба утверждения истинными; иначе метка сегмента это голосование, выданное за факт. Короткие прогоны поглощаются, иначе одно неверное слово кромсает фразу |
| 39 | **`num_speakers` — подсказка, а не гарантия** | sherpa строит одну дендрограмму и режет её либо по высоте (`threshold`), либо ровно на N листьев (`cutree_k`). Верхнее деление отделяет выброс, а не двух собеседников: измерено — на паре похожих голосов `num_speakers=2` даёт одного спикера, а порог 0.4 даёт двух. Поэтому счёт передаётся как есть, а расхождение называется варнингом `diarization_fewer_speakers` |
| 40 | **Модель эмбеддингов по умолчанию — VoxCeleb, а не zh/en** | Кластеризация сравнивает эмбеддинги между собой, но пространство обучено на корпусе. Измерено: `campplus-sv-zh-en` на паре русских голосов даёт один кластер при любом пороге от 0.30 до 0.60. `campplus-sv-voxceleb` их разделяет. Цена — на карикатурно разных голосах граница ставится хуже; это компромисс, а не чистый выигрыш |
| 41 | **`nanoasr init` — часть поставки, а не скрипт в документации** | Первый запуск требовал пяти шагов, из которых три механические. Команда пишет конфиг, выпускает admin- и user-ключ и качает четыре модели. Конфиг правится как YAML-документ (`config.Document`), поэтому `key issue`/`key remove` не стирают комментарии оператора |
| 42 | **Совместимость с whisper-asr-webservice — диалект, а не форк ядра** | Контракт того сервиса (`/asr`, `/detect-language`, `/asr_task`) воспроизведён пакетом `internal/api/era` поверх того же `core.Service`. Чего сборка не умеет — перевод — отвечается `501`, а не приблизительно; что нельзя менять на запрос — `encode`, `vad_filter` — принимается и называется в `X-NanoASR-Warnings`, потому что полей для предупреждений в заимствованном контракте нет |
...
- Каналы (`audio.channel_mode`): `downmix` (по умолчанию), `first`, `split`
  (каждый канал — отдельный проход, результаты мержатся по времени старта,
  `channel` и в сегментах, и в словах). `split` — это дешёвая альтернатива
  диаризации для двухканальной телефонии (A/B leg).

  Три режима — три разные команды ffmpeg, и путать их нельзя: `-ac 1`
  усредняет каналы (это `downmix`), `first` обязан **выбросить** лишние ноги
  и делается фильтром `pan=mono|c0=c0`, а `split` не конвертирует каналы
  вовсе и пишет WAV, который разбирает наш собственный декодер — число
  каналов должно приехать в самом потоке. До M5 `first` и `downmix` строили
  одинаковую команду и вели себя одинаково.

  Каналы обрабатываются **последовательно**: губернатор и так насыщает CPU,
  а второй канал в полёте удваивает пик памяти ровно на тех длинных файлах,
  где память и есть ограничение. Тишина и `speech_ratio` считаются по
  объединению каналов — тишина это интервал, где молчат все.

**Память:** 30 минут × 16000 × 4 байта = **115 МБ** на **канал** в пике. При
`channel_mode: split` умножается на число каналов: ни лимит загрузки, ни
байтовый бюджет очереди этого не знают — они считают приехавшее, а не то, во
что оно развернётся. Отсюда `audio.max_split_channels` (по умолчанию 2) и
`audio.max_decoded_bytes`, проверяемый **по ходу накопления**, той же
дисциплиной, что уже применялась к длительности.
...
|---|---|---|
| **Hotwords** | `HotwordsFile` + `HotwordsScore` (transducer), вариант пула | до декодирования, тайминги не трогает |
| **Пунктуация** | своя у модели, либо `OfflinePunctuation` (CT-Transformer) отдельной моделью | знаки приклеиваются к предыдущему слову, границы слов не сдвигаются |
| **Заглавные буквы** | из той же модели, что и знаки | посимвольная замена, тайминги неизменны |
...
разорвать связь «слово ↔ интервал времени». Контракт стадии —
`[]Word → ([]Word, spans)`, где `spans[i]` говорит, сколько подряд идущих
входных слов поглотило i-е выходное. Вектор нужен потому, что склейку надо
разложить обратно по сегментам, а по одному тексту не понять, какое слово
куда принадлежит; `Chain` проверяет, что сумма сходится — неверный вектор не
даёт видимой ошибки, он молча привязывает текст к чужому куску записи.

Цепочка идёт по **всему** транскрипту одним потоком, а не посегментно:
качество пунктуации зависит от контекста предложения, а предложение регулярно
пересекает границу VAD-сегмента. Склейка, пересёкшая границу, достаётся
сегменту, которому принадлежало её первое входное слово — и вообще возможна
только если граница была почти беззвучной, потому что правила отказываются
соединять слова, между которыми больше 0.35 с: число не произносят с паузой
посередине.

**Пунктуация и русский.** §5.6 писался в расчёте на CT-Transformer, но
русского CT-Transformer не существует — в зоопарке sherpa-onnx только zh-en и
en. Русскую пунктуацию даёт сама модель ASR: GigaAM v3 punct пишет знаки и
регистр (решение №36), и `punctuation_builtin` **измеряется** по её словарю,
а не заявляется (решение №37). Стадия `postproc.punctuation` остаётся для
моделей, которые сами не пунктуируют.

**Hotwords и что их ограничивает.** Биасинг работает только на transducer, только
с `modified_beam_search`, и только если фразу есть чем токенизировать: sherpa
принимает `bpe`, `cjkchar`, `cjkchar+bpe` — и **завершает процесс**, а не
возвращает ошибку, если дать ему `char`. Сегодня ни одна запись каталога этих
условий не выполняет, и сервер говорит об этом варнингом на каждый запрос.

ITN для русского в sherpa-onnx (`RuleFsts`) фактически отсутствует, поэтому слой правил
пишется свой: числа, даты, время, деньги, телефоны, проценты. Локали лежат за
реестром: добавить английский — это один файл и `init()`.
...
- Привязка к словам: для каждого слова берётся спикер с максимальным перекрытием
  ...
  спикер ближайшего по времени сегмента, поле `speaker_confidence` понижается до
  `FallbackConfidence`. При перекрытии `speaker_confidence` — это доля слова,
  которую покрыл выигравший тёрн: не вероятность, а способ отличить измеренную
  привязку от угаданной.
- **Сегменты режутся по границам тёрнов** (решение №38): §5.7 привязывает
  пословно, §6 держит спикера на сегменте, и резка — единственный способ
  сделать оба утверждения истинными. Прогон короче `MinTurnSec` (0.5 с) или
  из менее чем `MinTurnWords` (2) слов поглощается предыдущим: диаризатор
  время от времени отдаёт одно слово в середине фразы другому спикеру, и
  резать по нему значило бы превращать транскрипт в конфетти.
- `speakers[].total_speech` считается по сегментам транскрипта, а не по сырым
  тёрнам: тёрн, для которого распознавание не дало слов, — это не речь, которую
  кто-то видит, и его учёт рассинхронизировал бы сводку с текстом рядом.
- **Прогон, слишком короткий для тёрна, поглощается соседним в обе стороны.**
  Поглощение «в предыдущий» не покрывает первый прогон сегмента — ему некуда
  поглощаться, — и одно неверно атрибутированное первое слово становилось
  сегментом в 0.1 с. Первый прогон присоединяется к следующему, а спикера
  сегмент берёт не у первого слова, а у того, кто держит большую часть его
  времени: иначе поглощённая ошибка переименовывала бы всю фразу.
- **`num_speakers` не гарантирует N спикеров** (решение №39). Кластеризация —
  одна дендрограмма complete linkage; `num_speakers` меняет лишь место реза.
  Если запрошено больше, чем разделилось, ответ несёт
  `diarization_fewer_speakers` с обоими числами и с тем, что можно поменять.
  Код намеренно не оканчивается на `_unavailable`: транскрипт получен.
- **Частота дискретизации проверяется на старте.** sherpa не сверяет её ни с
  чем: подать 8 кГц в 16-килогерцовую модель — не ошибка, а запись, проигранная
  вдвое быстрее, с границами не на месте и эмбеддингами, которые склеиваются в
  одного спикера. `buildDiarizer` сверяет `SampleRate()` моделей с
  `audio.target_sample_rate` и отказывается стартовать при расхождении.
- Стоимость: SPEC закладывал ~0.3–0.5× RTF сверх распознавания; **измерено
  0.054×** на стенде M5 (`TestM5Report`). Диаризация **не** запускается, если
  `channel_mode: split` — там спикеры уже разделены каналами; это пропуск, а не
  отказ, поэтому код варнинга `diarization_skipped_split` намеренно не
  оканчивается на `_unavailable` и не валит strict-режим.
- Отмены внутри прохода нет (решение №34): `ProcessWithCallback` есть в C-API и
  не экспортирован Go-обёрткой.
  ...
  "segments": [
    {
      ...
      "words": [
        { "word": "здравствуйте", "start": 1.28, "end": 1.96, "confidence": 0.91,
          "speaker": "spk_0", "speaker_confidence": 0.97 },
        { "word": "это",          "start": 2.02, "end": 2.14, "confidence": 0.88,
          "speaker": "spk_0", "speaker_confidence": 1.0 }
      ]
    }
  ],
...
- `speaker` — `null`, а не пустая строка, если диаризация не запускалась. На
  словах он `omitempty`: одноканальный моноспикерный транскрипт сериализуется
  ровно так же, как до появления диаризации.
- `stages_ms` всегда несёт ключи `post` и `diarize`, даже нулями: клиент не
  отличит отсутствующий ключ от стадии, которая не запускалась, а 0 говорит
  «не запускалась» однозначно.
- `segments_total` — число **VAD**-сегментов. `len(segments)` — то, что осталось
  после резки по спикерам и слияния каналов, и это разные числа.
- `channel` есть и на сегменте, и на слове; при `channel_mode: split` сегменты
  обоих каналов смержены по времени старта и перенумерованы.
...
**Возможностей в манифесте нет.** Что модель умеет, следует из её семейства и
из неё самой, а манифест, заявивший иное, был бы утверждением, которое никто не
проверял. `word_timestamps` и `confidence` даёт семейство;
`punctuation_builtin` **измеряется** по `tokens.txt` при загрузке (решение
№37) — из семейства он не выводится, потому что GigaAM v2 и v3 это одно
семейство и различаются ровно этим, а словарь пунктуирующей модели отличается
наблюдаемо: 257 подслов со знаками и заглавными против 34 строчных букв.
...
**Варианты в пуле.** Ключ пула — это id модели, а для запроса, потребовавшего
собственной конфигурации распознавателя (hotwords, `decoding_method`,
`max_active_paths`), — `id!<хеш варианта>`. Такие записи вытесняются раньше
базовых: базовая обслуживает обычный трафик, вариант восстановим из неё.
Бюджет `asr.variants.max` считается **на базовую модель**, и его исчерпание
даёт варнинг, а не вытеснение: клиент, шлющий новый список hotwords на каждый
запрос, иначе гонял бы пул по кругу. Hot swap снимает и варианты заменяемой
модели — иначе запрос с hotwords продолжил бы декодировать на весах, которые
свап и должен был убрать.
...
Из них `vad` не реализован; остальные исполняются либо сопровождаются варнингом,
называющим причину и настройку, которая бы это исправила.
...
### 8.3 Диалект `era` — контракт whisper-asr-webservice

Совместимость с [whisper-asr-webservice](https://github.com/ahmetoner/whisper-asr-webservice)
и его Era-форком: клиент, написанный под тот сервис, переключается на NanoASR
без правок. Пути, имена параметров, конверты ответов, заголовки `Asr-Engine` и
`Content-Disposition` и `text/plain` у **всех** пяти форматов — как там.

| Метод | Путь | Заметки |
|---|---|---|
| `POST` | `/asr` | multipart `audio_file`; query: `encode`, `task`, `language`, `initial_prompt`, `vad_filter`, `word_timestamps`, `diarize`, `min_speakers`, `max_speakers`, `output` |
| `POST` | `/detect-language` | multipart `audio_file`; `{detected_language, language_code, confidence}` |
| `POST` | `/asr_task` | те же параметры, что у `/asr`; `{success, task_id}` |
| `GET` | `/asr_task/{task_id}` | не готово → `{success:true, is_ready:false}`; готово → сам транскрипт; упало → `{success:false, is_ready:true, error:{title,message,traceback}}` |

`output`: `txt` (по умолчанию), `vtt`, `srt`, `tsv`, `json`. `tsv` — заголовок
`start/end/text` и целые миллисекунды, как у writer'ов whisper.

Расхождения — каждое намеренное и задокументированное в коде пакета:

| Что | Почему |
|---|---|
| `GET /` не редиректит на `/docs` | Swagger UI здесь нет, а диалект, забравший корень, отвечал бы за все непойманные пути — и за чужие диалекты тоже |
| `task=translate` → `501` | моделей перевода в сборке нет (решение №9 не делает диалект умнее ядра) |
| `initial_prompt` → hotwords | ровно как `prompt` в диалекте `openai` |
| `encode`, `vad_filter` принимаются и не действуют | это настройки сервера, а не опции запроса |
| `min_speakers`/`max_speakers` → одно число | кластеризатор берёт точный счёт или порог; диапазону не на чем резать (решение №39) |
| `task_id` = `<job_id>~<output>` | формат нужен при опросе, а таблица в памяти терялась бы при рестарте — притом что задача его переживает |
| аутентификация обязательна | как у всего остального сервера; у upstream её нет вовсе |
| задачи живут `jobs.history_ttl` | upstream удаляет их через 1800 с; наши переживают рестарт |

Предупреждений в этом контракте нет ни одного поля: `/asr` отдаёт непрозрачное
тело, `/asr_task` — success и результат. Поэтому деградации едут в заголовке
`X-NanoASR-Warnings` (список кодов через запятую): правило «никогда не
деградировать молча» держится и в заимствованном контракте, а клиент, который
про этот заголовок не знает, ничего не замечает.

`/detect-language` стоит целого распознавания, а не одного прохода энкодера по
30 секундам, как у upstream. Модели идентификации языка в сборке нет; ответ —
язык, который объявляет разрешившаяся модель, а `confidence` говорит, был ли
выбор: `1`, когда модель не умеет ничего другого, и `0`, когда языков в
манифесте несколько и endpoint взял первый. Узнать, какая модель разрешилась,
можно только распознав файл — иначе диалект отрастил бы вторую копию правил
выбора модели.

### 8.4 Добавление своего диалекта
...
### 10.1 Команды, которые пишут конфиг (решение №41)

```
nanoasr init  [-config FILE] [-data-dir DIR] [-addr ADDR] [-model ID]
              [-no-diarize] [-no-download] [-force]
nanoasr key   list | issue NAME [-admin] [-interactive] [-rps N] [-hash] | remove NAME
```

`init` пишет конфиг, выпускает **admin**- и **user**-ключ и качает четыре модели
по умолчанию (ASR + VAD + segmentation + embedding). Порядок обязателен: файл
пишется **до** загрузки моделей и сразу перечитывается через `config.Load`, чтобы
команда не могла оставить конфиг, который сервер откажется читать; неудачная
загрузка весов после этого — уже отдельная ошибка с отдельным советом
(`nanoasr models pull`), а не «init не сработал».

`-data-dir` по умолчанию `/var/lib/nanoasr`, если туда можно писать, иначе
`./nanoasr-data`. Выбор печатается, а не подразумевается.

Ключи хранятся открытым текстом (`key list` их показывает замаскированными) либо,
с `-hash`, только как `sha256:` — тогда секрет печатается один раз. Это ровно те
две формы, которые уже понимает загрузчик (§10, `auth.keys`).

**Редактирование сохраняет комментарии.** Файл разбирается в `yaml.Node`
(`config.Document`), а не в `Config`: круг «прочитать в структуру — записать
обратно» работал бы один раз, а на второй стёр бы комментарии оператора, переставил
ключи в порядок полей структуры и записал бы в файл каждый дефолт, который
подставил загрузчик, — как будто его выбрали. Пустые строки между блоками
восстанавливаются отдельно: yaml.v3 их не хранит, а без них файл превращается в
сплошную стену. Запись атомарна (временный файл рядом плюс `rename`) и
предварительно перечитывается: правка, породившая нечитаемый YAML, падает до того,
как заменит рабочий файл.
...
audio:
  ...
  max_split_channels: 2            # split декодирует каждый канал целиком
  max_decoded_bytes: 0             # 0 → max_duration × rate × 4 × max_split_channels
...
asr:
  ...
  default_model: gigaam-v3-ctc-punct-ru   # решение №36; пусто → единственная загруженная или ошибка
  ...
  # Второй резидентный экземпляр под per-request конфигурацию распознавателя.
  # 0 → не допускать; запрос получает настройки модели и варнинг (решение №35).
  variants: { max: 0, allow_hotwords: false }
...
postproc:
  punctuation: { enabled: false, model: "" }   # только для моделей без своей
  itn:         { enabled: false, locale: ru }  # модель не нужна, правила вшиты
  hotwords:    { enabled: false, default_score: 1.5 }

diarization:
  enabled: false
  segmentation_model: pyannote-segmentation-3
  embedding_model: campplus-sv-voxceleb   # VoxCeleb, не zh/en — решение №40
  # 0.5 — дефолт sherpa; слишком снисходителен к двум голосам одного языка.
  clustering: { num_clusters: 0, threshold: 0.4 }
  min_duration_on: 0.3
...
Под `channel_mode: split` пиковый PCM на задачу умножается на
`max_split_channels`, и `max_concurrent` делится на него: план, не учитывающий
этого, — фикция. Отдельный запрос со `split` на downmix-сервере всё ещё может
временно превысить план на `(каналы−1) × 115 МБ × max_concurrent`, и границей
служит `max_split_channels`.
...
| Нагрузка | `scripts/loadtest.sh` (`make load`): 100 параллельных 5-минутных файлов через HTTP, тридцать минут. Ничего не утверждает — печатает RSS во времени, потому что утечку видно только так. Не в CI: тридцать минут по замыслу |
| Golden ITN | `internal/postproc/itn/testdata/ru.golden`, 33 случая. Половина из них фиксирует, что́ переписывать **не** надо: «один вопрос» не должен стать «1 вопрос», час без минут — не время, девять цифр — не телефон. Слишком усердная нормализация это тот отказ, из-за которого фичу выключают |
| Отчёты | `TestM1Report` и `TestM5Report` ничего не утверждают: числа зависят от железа, и порог врал бы на ноутбуке и на сервере по-разному |
...
│   │   ├── era/            # диалект whisper-asr-webservice
...
| **M5 — качество** ✅ | `channel_mode: split` с бюджетом памяти, диаризация со спикерами на словах и резкой сегментов, пунктуация (своя у модели + CT-Transformer для чужих), ITN(ru) за реестром локалей с golden-корпусом, hotwords как вариант пула, нагрузочный скрипт и отчёт M5. Три бага M1–M3 попутно: мёртвая ветка `first` в ffmpeg, проверка split по неслитому режиму, потерянный `decoding_method` | 8 → 12 факт |
...
Итого v1 (M0–M5): **≈42 человеко-дня**. Критический путь — M1 → M2 → M3; M4 можно
...
23. Нужны ли **`srt`/`vtt`** с пословной разбивкой (karaoke-style) или посегментной?
    *Частично закрыто в M5:* разбивка посегментная, и при диаризации несёт
    спикера — WebVTT штатным `<v spk_0>`, SubRip префиксом, поскольку такого
    тега у него нет.
...
30. ~~Какой набор **языков ITN** нужен реально — только ru, или ещё en?~~
    **Закрыт в M5:** реализован ru, локали лежат за реестром — en добавляется
    одним файлом и `init()`, без правки движка.
